...
import { MapPin, MapPinned, UserRound } from "lucide-react";
                ...
                <div className='grid sm:grid-cols-2 grid-cols-1 mx-3 md:mx-5 lg:mx-10 xl:grid-cols-4 md:grid-cols-2 lg:grid-cols-3 gap-5 '>
                    {
                        restaurants?.map((restaurant, idx) => (
                            <div
                                key={idx}
                                className={`border-2 pb-2 transition-all duration-200  rounded-md ${theme === 'light' ? 'border-gray-100 hover:shadow hover:shadow-customOrange/40' : 'border-zinc-700 hover:shadow-2xl hover:shadow-customOrange/20'}`}
                            >
                                ...
                                <div>
                                    ...
                                    <div className=" relative">
                                        <img src={restaurant?.image.url} alt="restaurant img" className="w-full  max-h-50  object-cover  transition-all rounded-tl-md rounded-tr-md duration-300 overflow-hidden" />
                                    </div>

                                    {/* restaurant glance data :  */}
                                    <div className={` mx-2  rounded-md  py-4 ${theme === 'dark' && 'bg-zinc-800'}`}>

                                        {/* restaurant icon and title :  */}
                                        <div className={`flex items-center gap-1`}>
                                            <div className={`border rounded-full py-px px-px ${theme === 'light' ? 'bg-orange-50 border-orange-100' : 'bg-customOrange border-orange-700'}`}>
                                                <GrRestaurant className={`${theme === 'light' ? 'text-customOrange' : 'text-orange-200'}`} />
                                            </div>
                                            <h2 className='md:text-lg text-sm  font-semibold'>{restaurant?.shopName}</h2>
                                        </div>




                                        <div className="grid grid-cols-1 gap-2 text-[10px] md:text-xs mt-2 text-gray-400">
                                            ...
                                            <div className='flex items-center gap-1  '>
                                                ...
                                                <span >Address line 1 : {restaurant?.address1}</span>
                                            </div>

                                            <div className='flex items-center gap-1  '>
                                                <MapPin className="size-4" />
                                                <span >Address line 2 : {restaurant?.address2}</span>
                                            </div>
                                    ...
                                    <Button
                                        className=" float-right mr-2 rounded bg-customOrange"
